		...
		<div id="comment-<?php comment_ID(); ?>" class="comment-wrapper">
			<div class="comment-meta">
				...
				<div class="comment-author vcard">
					<strong class="fn"><?php comment_author_link(); ?> a dit: </strong>
				</div><!-- .comment-author .vcard -->
			</div>
			<div class="comment-entry">
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Votre commentaire est en attente de moderation', 'modularity' ); ?></em>
					<br />
				<?php endif; ?>
				<?php comment_text(); ?>
				<div class="fn">				
					Le <?php printf( __( '%1$s à %2$s', 'modularity' ), get_comment_date("l d F Y"),  get_comment_time() ); ?></a>	
				</div>
				<div class="reply">
					<?php comment_reply_link( array_merge( $args, array( 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?> <?php edit_comment_link( __( '(Editer le commentaire)', 'modularity' ), ' ' ); ?>
				</div><!-- .reply -->				
			</div>
	</div><!-- #comment-##  -->
